...
from ..static.python.departamentos import (
...
class Departamentos(APIView):
    ...
    def post(self, request):
        ...
        if len(data_auth )>0 and token:
            data = departamentos.get_info(request.data.get("clave"))   
            return Response({"data": data},status=status.HTTP_200_OK)
        return Response({},status=status.HTTP_403_FORBIDDEN)
class DepartamentosCards(APIView):   
    authentication_classes = ()
    permission_classes = ()
    def post(self, request):
        token = request.data.get("token")
        data_auth = token_auth.token_auth(token)
        if len(data_auth )>0 and token:
            data = registro_util.cards(request.data.get("buscador"),request.data.get("limit"),request.data.get("value"),"departamentos","departamentoID","nombre")     
            return Response({ "data": data},status=status.HTTP_200_OK)
        return Response({},status=status.HTTP_403_FORBIDDEN)    
class DepartamentosDashboard(APIView):
    permission_classes = ()
    authentication_classes = ()
    def post(self, request):
        data = []
        ...
        return Response({},status=status.HTTP_403_FORBIDDEN)      
class DepartamentosGeneral(APIView):
    permission_classes = ()
    authentication_classes = ()
    def post(self, request):
        token = request.data.get("token")
        ...
        if len(data_auth )>0 and token:
            ...
            return Response({ "data": data},status=status.HTTP_200_OK)  
        return Response({},status=status.HTTP_403_FORBIDDEN)  
class DepartamentosRegistros(APIView):
    permission_classes = ()
    authentication_classes = ()
    def post(self, request):
        token = request.data.get("token")
        ...
        if len(data_auth )>0 and token:
            ...
            return Response({ "data": data},status=status.HTTP_200_OK)  
        return Response({},status=status.HTTP_403_FORBIDDEN)  
class DepartamentosForm(APIView):
    permission_classes = ()
    authentication_classes = ()
    def post(self, request):
        ...
        if len(data_auth )>0 and token:
            ...
            if registro_util.codigo_existente(data['nombre'],data['nombre'],"departamentos","nombre","AGREGANDO"):
                departamentos.agregando(data,data_auth['usuarioID'])
    ...
    def patch(self, request):
        ...
        if len(data_auth )>0 and token:
            if request.data.get("opcion")=="EDITANDO":
                ...
                if registro_util.codigo_existente(request.data.get("nombre"),data_info["nombre"],"departamentos","nombre","EDITANDO"):
                    departamentos.editando(data_info,request.data.get("clave"),data_auth['usuarioID'])
                ...
                data = departamentos.eliminando(request.data.get("clave"))
