...
from ..static.python.generos import (
...
class Generos(APIView):
    ...
    def post(self, request):
        ...
        if len(data_auth )>0 and token:
            data = generos.get_info(request.data.get("clave"))   
            return Response({"data": data},status=status.HTTP_200_OK)
        return Response({},status=status.HTTP_403_FORBIDDEN)
class GenerosCards(APIView):   
    authentication_classes = ()
    permission_classes = ()
    def post(self, request):
        token = request.data.get("token")
        data_auth = token_auth.token_auth(token)
        if len(data_auth )>0 and token:
            data = registro_util.cards(request.data.get("buscador"),request.data.get("limit"),request.data.get("value"),"generos","generoID","nombre")     
            return Response({ "data": data},status=status.HTTP_200_OK)
        return Response({},status=status.HTTP_403_FORBIDDEN)    
class GenerosDashboard(APIView):
    permission_classes = ()
    authentication_classes = ()
    def post(self, request):
        data = []
...
class GenerosGeneral(APIView):
    ...
    def post(self, request):
        ...
        if len(data_auth )>0 and token:
            ...
            return Response({ "data": data},status=status.HTTP_200_OK)  
        return Response({},status=status.HTTP_403_FORBIDDEN)  
class GenerosRegistros(APIView):
    permission_classes = ()
    authentication_classes = ()
    def post(self, request):
        token = request.data.get("token")
...
class GenerosForm(APIView):
    ...
    def post(self, request):
        ...
        if len(data_auth )>0 and token:
            ...
            if registro_util.codigo_existente(data['nombre'],data['nombre'],"generos","nombre","AGREGANDO"):
                generos.agregando(data,data_auth['usuarioID'])
    ...
    def patch(self, request):
        ...
        if len(data_auth )>0 and token:
            if request.data.get("opcion")=="EDITANDO":
                ...
                if registro_util.codigo_existente(request.data.get("nombre"),data_info["nombre"],"generos","nombre","EDITANDO"):
                    generos.editando(data_info,request.data.get("clave"),data_auth['usuarioID'])
                ...
                data = generos.eliminando(request.data.get("clave"))
