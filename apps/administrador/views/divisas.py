...
from ..static.python.divisas import (
 
    divisas,
   
)

class Divisas(APIView):
    ...
    def post(self, request):
        ...
        if len(data_auth )>0 and token:
            data = divisas.get_info(request.data.get("clave"))   
    ...
    def patch(self, request):
        ...
        if len(data_auth )>0 and token:
            ...
            divisas.editando(data_info,request.data.get("clave"),data_auth['usuarioID'])
